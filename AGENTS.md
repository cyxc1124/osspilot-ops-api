# osspilot-ops-api

运营 API。对照 OssPilot `legacy/`（v0.6.0）按切片用 Go 重写，不抄 Python。

## 提交

`<type>: <中文说明>`，可加范围：`feat(auth): ...`。必须保留 type 前缀，不要只写中文。

- `feat` 新功能
- `fix` 修缺陷
- `docs` 文档
- `style` 格式（不影响行为）
- `refactor` 重构
- `perf` 性能
- `test` 测试
- `build` 构建与依赖
- `ci` CI / 工作流
- `chore` 脚手架、杂项
- `revert` 回滚

```
feat: 实现运营登录、当前用户和修改密码
fix: 改密后清除 must_change_password
docs: 扩充提交前缀
ci: 增加 healthz 与 Lint/Test/Build
chore: 初始化仓库
```

## 实现

- Go 1.26，`net/http` ServeMux，`pgx/v5`，goose embed。不用 Gin / GORM。启动不做 migrate。
- 一片一变，过 CI 再合。不要顺手做邻片。
- 有新表才加 `migrations/*.sql`。库还没跑过时，改建表脚本，不要补清理迁移。
- `00001` 建表时写入种子 `admin` / `admin`（`must_change_password=true`）。首次登录后除改密、`/api/me`、登出外一律 403。
- 暂时不写功能测试；只留能挡住编译的最小检查。
- 错误体 `{"detail":"..."}`。默认端口 `:8001`。
- 前端不打进 API 镜像。
